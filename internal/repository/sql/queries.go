...
// Binary file queries.
var InsertBinaryFile = `INSERT INTO binary_files (user_id, name, file, extension, updated_at, meta) VALUES ($1, $2, $3, $4, $5, $6)`

var GetBinaryFile = `SELECT user_id, name, file, extension, updated_at, meta FROM binary_files WHERE user_id = $1 AND name = $2`

var UpdateBinaryFile = `UPDATE binary_files SET user_id=$1, name=$2, file=$3, extension=$4, updated_at=$5, meta=$6 WHERE user_id = $1 AND name=$2`

var DeleteBinaryFile = `DELETE FROM binary_files WHERE user_id = $1 AND name = $2`

var ListBinaryFiles = `SELECT name, file, extension, updated_at, meta FROM binary_files WHERE user_id = $1`
